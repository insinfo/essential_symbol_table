...
class SymbolTable<T> {
  ...
  Variable<T> create(String name, {T? value, bool? constant}) {
    ...
    if (_variables.any((v) => v!.name == name))
      throw new StateError('A symbol named "$name" already exists within the current context.');
  ...
  Variable<T>? remove(String name) {
    ...
    while (search != null) {
      Variable<T>? variable = search._variables.firstWhere((v) => v!.name == name, orElse: () => null);
  ...
  Variable<T>? resolve(String name) {
    var v = _lookupCache.putIfAbsent(name, () {
      var variable = _variables.firstWhere((v) => v!.name == name, orElse: () => null);
